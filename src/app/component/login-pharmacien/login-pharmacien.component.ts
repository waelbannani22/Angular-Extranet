...
export class LoginPharmacienComponent implements OnInit {
  ...
  login() {
    ...
    this.pharmacienService
      .login(this.pharamacien.email, this.pharamacien.password)
      .subscribe(
        (res) => {
          ...
          if (res.status == 200) {
            ...
            this.roles = this.tokenStorage.getUser().roles;
            if(res.body.user.role==="PHARMACIEN" || res.body.user.role==="OPTICIEN" ){
        ...
        (err) => {
          if (err.status == 405) {
            this.toastr.error(
              ...
              "veuillez attendre que l'administrateur vous vérifie!",
              {
                timeOut: 3500,
              }
            );
